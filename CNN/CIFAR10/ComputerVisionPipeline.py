# quick_pipeline.py  ────────────────────────────────────────────────
import cv2, torch
from torch import nn, optim
from torch.utils.data import DataLoader
from torchvision import datasets, io, transforms, models
from pathlib import Path

# --- change these three lines and you’re good ----------------------
ROOT    = r"D:\data\flowers"     # root/train  root/val  folder-per-class
BATCH   = 32
EPOCHS  = 3                      # bump if you have time
# -------------------------------------------------------------------

device = "cuda" if torch.cuda.is_available() else "cpu"

# --- OpenCV pre-processing (exactly your steps) --------------------
def cv2_prep(t):                                 # t: uint8 CHW RGB
    img = t.permute(1, 2, 0).numpy()             # HWC
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    img = cv2.resize(img, (0, 0), fx=4, fy=4, interpolation=cv2.INTER_CUBIC)
    img = cv2.fastNlMeansDenoisingColored(img, None, 10, 10, 7, 21)
    k   = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    img = cv2.dilate(img, k, 1);   img = cv2.erode(img, k, 1)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    return torch.tensor(img).permute(2, 0, 1)    # CHW uint8
# ------------------------------------------------------------------

SIZE = 299        # big enough for Inception
tfm  = transforms.Compose([
    cv2_prep,
    transforms.ConvertImageDtype(torch.float32),     # → 0-1
    transforms.Resize(SIZE), transforms.CenterCrop(SIZE),
    transforms.Normalize((0.485, 0.456, 0.406),
                         (0.229, 0.224, 0.225))
])

train = datasets.ImageFolder(Path(ROOT)/"train", loader=io.read_image, transform=tfm)
val   = datasets.ImageFolder(Path(ROOT)/"val",   loader=io.read_image, transform=tfm)

train_loader = DataLoader(train, BATCH, shuffle=True,  num_workers=2, pin_memory=True)
val_loader   = DataLoader(val,   BATCH, shuffle=False, num_workers=2, pin_memory=True)

nclass = len(train.classes)

# --- build 4 models + replace final layer --------------------------
def new_head(m, in_feat):             # helper
    return nn.Linear(in_feat, nclass)

models4 = [
    models.resnet50(weights="IMAGENET1K_V2"),
    models.swin_t(weights="IMAGENET1K_V1"),
    models.vit_b_16(weights="IMAGENET1K_V1"),
    models.inception_v3(weights="IMAGENET1K_V1", aux_logits=False)
]

# plug new heads
models4[0].fc              = new_head(models4[0], models4[0].fc.in_features)
models4[1].head            = new_head(models4[1], models4[1].head.in_features)
models4[2].heads.head      = new_head(models4[2], models4[2].heads.head.in_features)
models4[3].fc              = new_head(models4[3], models4[3].fc.in_features)

for m in models4: m.to(device)

# --- tiny training loop -------------------------------------------
loss_fn = nn.CrossEntropyLoss()
for m in models4:
    opt = optim.AdamW(m.parameters(), lr=3e-4)
    for _ in range(EPOCHS):
        m.train()
        for x, y in train_loader:
            x, y = x.to(device), y.to(device)
            opt.zero_grad();  loss_fn(m(x), y).backward();  opt.step()

# --- soft-vote ensemble -------------------------------------------
class Ensemble(nn.Module):
    def __init__(self, nets): super().__init__(); self.nets = nn.ModuleList(nets)
    def forward(self, x):
        return torch.stack([torch.softmax(n(x), 1) for n in self.nets]).mean(0)

ens = Ensemble(models4).to(device)

# quick val accuracy
def accuracy(model):
    model.eval(); hits = tots = 0
    with torch.no_grad():
        for x, y in val_loader:
            x, y = x.to(device), y.to(device)
            hits += (model(x).argmax(1) == y).sum().item();  tots += y.size(0)
    return hits / tots

print(f"Ensemble val-acc: {accuracy(ens):.3f}")

